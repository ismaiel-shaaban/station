...
import { getMerchantByIdService, updateMerchantService, getMerchantTypesService } from "@/redux/services/merchants";
...
export default function EditMerchant() {
  ...
  const { merchantData, merchantTypesList, isLoading } = useSelector((state) => state.merchants);
  ...
  useEffect(() => {
    if (merchantData) {
      setValue("username", merchantData.username || "");
      setValue("name", merchantData?.name || "");
      setValue("logo", merchantData?.logo || "");
      setValue("phone", merchantData?.phone || "");
      setValue("merchantTypeId", merchantData?.merchantTypeId || "");
      setValue("profitPercentage", merchantData?.profitPercentage || "");
      setLogoPreview(merchantData?.logo ? `https://us-east-1.linodeobjects.com/fantaza/${merchantData.logo}` : null);
    }
  ...
  const handleLogoChange = async (e) => {
    ...
    if (file) {
      ...
      try {
        const response = await dispatch(uploadFileService({formData , bucketName: merchantData?.bucketName })).unwrap();
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await dispatch(updateMerchantService({ id: params.id, data: payload })).unwrap();
                  ...
                  <div className="row">
                    ...
                    <div className="col-sm-6 mb-20 position-relative">
                      <label htmlFor="password" className="form-label fw-semibold text-primary-light text-sm mb-8">
                        كلمة المرور <span className="text-secondary">(اختياري)</span>
                      </label>
                      <div className="position-relative">
                        <input
                          ...
                          id="password"
                          placeholder="ادخل كلمة مرور جديدة (اتركها فارغة للاحتفاظ بالحالية)"
                          {...register("password")}
                        />
                        <span
